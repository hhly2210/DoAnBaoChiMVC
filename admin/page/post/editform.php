...
<div class="card box round first grid">
	...
	<div class="card-body" id="ModalEdit">
		<form id="edit-form" action="api/edit.php" method="POST" enctype="multipart/form-data">
			...
			<div class="row mb-3">
				...
				<div class="col-sm-10">
					<div class="input-group input-group-merge">
						<button class="btn btn-outline-primary" type="button" id="button-addon1" onclick="elfinderDialog2('#Images')">Thêm ảnh
						</button>
	...
	<script>
		function sua(id) {
			...
			fetch('/admin/api/post/get.php?postID=' + id)
				.then(res => {
					if (res.status === 200) {
						res.json().then(obj => {
							form.elements.Title.value = obj.Title
							form.elements.Abstract.value = obj.Abstract
							$('#summernote').summernote('code', obj.Contents)
							form.elements.catID.value = obj.catID
							form.elements.Images.value = obj.Images
						})
					}
				})
		}
